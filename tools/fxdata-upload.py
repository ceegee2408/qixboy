...
forced_port = os.environ.get("FX_UPLOAD_PORT", "").strip()
...
def getComPort(verbose):
  ...
  if forced_port:
    bootloader_active = False
    if verbose : sys.stdout.write("Using configured upload port {} ".format(forced_port))
    return forced_port
...
def bootloaderStart():
  ...
  # With a forced COM port we cannot infer mode from VID/PID, so always trigger
  # the 1200bps reset and reconnect to the same port.
  if forced_port:
    try:
